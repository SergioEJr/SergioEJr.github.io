---
title: "A picture proof of the product rule"
description: "The product rule falls out of a rectangle — no calculus required."
pubDate: "2023-07-09"
category: math
authors:
  - sergio-eraso
tags:
  - calculus
  - derivatives
draft: false # set to true to unpublish
---

import Figure from '../../components/Figure.astro';
import productRule from '../../assets/blog/product-rule.svg?raw';
import cubeX3 from '../../assets/blog/cube-x3.svg?raw';

We want the derivative of a product $f \cdot g$, which we can picture as the area
of a rectangle with sides $f$ and $g$. Nudging the input by a small amount grows each
side by its own differential, $df$ and $dg$:

<Figure
  svg={productRule}
  width={360}
  alt="A rectangle of area f·g with two thin strips added along its edges"
  caption="The change in area is the two strips plus a vanishing corner."
/>

The new area exceeds the old by two thin strips plus a small corner. The strips
have areas $g \,df $ and $f\,dg$, whereas the corner $df\,dg$ is a
second order contribution and drops out in the limit. What
survives is exactly

$$
d(fg) = f \, dg + g \,df
$$

the product rule.

The same picture works one dimension up. Take $x^3$ as the volume of a cube of
side $x$, and grow the side by $dx$. The three faces you can see each grow
outward into a slab of volume $x^2\,dx$, so the volume gains $3x^2\,dx$, the
derivative $3x^2$. What's left over is thinner still: three long edges of order
$x\,dx^2$ and a single corner cube $dx^3$. Those are higher order and vanish in
the limit, exactly like the corner did for the rectangle.

<Figure
  svg={cubeX3}
  width={520}
  alt="A cube of volume x³ growing by dx on three faces: three x²dx slabs plus a faint higher-order corner"
  caption="Growing the cube adds three x²dx slabs — the derivative 3x² — plus the faint dx³ corner that vanishes."
/>
