...
<Figure
  ...
  caption="To transform a vector field, both the positions $\boldsymbol{x}$ and the vectors $\boldsymbol{m}$ must rotate together (bottom right). Rotating only one leaves the magnetization misaligned with the bar."
/>
