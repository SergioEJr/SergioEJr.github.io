...
% alt: Left: a curved velocity field v, with a curve gamma of t that cuts
%   across the neighbouring streamlines but runs tangent to the one through
%   the point x, where its tangent vector is v(x). An arrow labelled Phi above
%   and J of x below carries this to the right panel: a straightened field
%   v-prime in which the image curve Phi of gamma of t is likewise tangent at
%   x-prime, with tangent vector v-prime(x-prime).
% width: 550
\ifnum\pdfoutput>0
...
\begin{tikzpicture}[x=1mm, y=1mm]
  ...
  % Labels. \boldsymbol{v} sits just off the top end of the outermost arc, so it reads as
  % naming the bundle rather than floating in the margin.
  \node[lbl, text=ink,     anchor=east]  at (9,50.5)          {$\boldsymbol{v}$};
  \node[lbl, text=figred,  anchor=south] at (15,54)           {$\gamma(t)$};
  \node[lbl, text=figred,  anchor=east]  at ({\xax-2.1},{\xay})  {$\boldsymbol{x}$};
  ...
  \node[lbl, text=figblue, anchor=west]  at (34.5,10)         {$\boldsymbol{v}(\boldsymbol{x})$};
  ...
  \node[lbl, text=figblue, anchor=north] at (68,25.8) {$J(\boldsymbol{x})$};
  ...
  \begin{scope}[shift={(86,0)}]
    ...
    % Labels
    \node[lbl, text=ink,     anchor=east]  at (5.5,50.5)          {$\boldsymbol{v}'$};
    \node[lbl, text=figred,  anchor=south] at (17,53)             {$\Phi(\gamma(t))$};
    \node[lbl, text=figred,  anchor=east]  at ({\xbx-2.1},{\xby}) {$\boldsymbol{x}'$};
    \node[lbl, text=ink,     anchor=west]  at ({\xbx+2.1},{\xby}) {$t=0$};
    \node[lbl, text=figblue, anchor=west]  at (45,7)              {$\boldsymbol{v}'(\boldsymbol{x}')$};
  \end{scope}
\end{tikzpicture}
\end{document}
